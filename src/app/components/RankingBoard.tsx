...
export function RankingBoard({ showCopyLink = false }: RankingBoardProps) {
  ...
  const searchedMaleRanking = searchQuery
    ? maleRanking.filter((entry) => entry.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : maleRanking;
  
  const searchedFemaleRanking = searchQuery
    ? femaleRanking.filter((entry) => entry.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : femaleRanking;
